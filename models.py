...
class EvaluationResponse(BaseModel):
    """Response model for individual evaluation"""
    id: int = Field(..., description="Evaluation ID")
    knowledge_id: int = Field(..., description="Associated knowledge item ID")
    ...
    # Knowledge item details (from join)
    knowledge_item: Optional[Dict[str, Any]] = Field(None, description="Associated knowledge item details")


class EvaluationsResponse(BaseModel):
    """Response model for list of evaluations"""
...
class EvaluationStatsResponse(BaseModel):
    ...
    category_performance: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Performance by academic category")
    recent_evaluations: int = Field(..., description="Number of evaluations in last 7 days")
    highest_scoring_category: Optional[str] = Field(None, description="Category with highest average score")
    lowest_scoring_category: Optional[str] = Field(None, description="Category with lowest average score")
    status: str = Field("success", description="Request status")